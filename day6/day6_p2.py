...
def move_guard(guard_map, guard_x, guard_y, dir_cnt):
    ...
    if guard_map[next_pos[0]][next_pos[1]] == '#':
        dir_cnt = (dir_cnt + 1) % 4
    return guard_x + directions[dir_cnt][0], guard_y + directions[dir_cnt][1], dir_cnt, False
...
def part2(data: list) -> int:
    ...
    while 0 <= guard_x < len(guard_map) and 0 <= guard_y < len(guard_map[0]):
        guard_x, guard_y, dir_cnt, out_of_bounds = move_guard(guard_map, guard_x, guard_y, dir_cnt)
        if out_of_bounds:
            break

        if guard_map[guard_x][guard_y] == '.':
            guard_map[guard_x][guard_y] = '#'
            cycle_count += cycle_check(guard_map, guard_x, guard_y, len(guard_map), len(guard_map[0]), dir_cnt)
            guard_map[guard_x][guard_y] = ''
